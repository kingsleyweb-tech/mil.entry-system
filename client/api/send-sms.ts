...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    const smsRes = await fetch(gatewayUrl, { method: 'GET' })

    const textResponse = await smsRes.text()
    console.log('[send-sms] gateway status:', smsRes.status, '| response:', textResponse)

    if (!smsRes.ok) {
      console.error('[send-sms] gateway rejected request:', smsRes.status, textResponse)
    ...
    console.error('[send-sms] Fetch error calling gateway:', err)
